...
export async function taskScanSitemaps(siteId) {
    const { site } = await loadSiteContext(siteId);
    await sitemaps.scanAllSitemaps(site);
}
export async function taskInspect(siteId, limit) {
    const { site, user } = await loadSiteContext(siteId);
    const summary = await indexer.inspectBatch(user.id, site, limit, "manual");
    await logEvent(`Inspekcja ${site.display_name}: sprawdzono ${summary.checked} URL-i (${summary.indexed} zaindeksowanych).`, { workspaceId: site.workspace_id, category: "inspection", details: summary });
}
export async function taskRunPipeline(siteId, scan = true) {
...
export async function taskSubmitUrls(siteId, urlIds) {
    const { site, workspace, user } = await loadSiteContext(siteId);
    let pages = [];
    if (urlIds.length > 0) {
        pages = await db
...
export async function taskInspectUrls(siteId, urlIds) {
    const { site, user } = await loadSiteContext(siteId);
    if (urlIds.length === 0)
        return;
...
export async function taskRunAllSites(workspaceId) {
    const workspace = await db
        .selectFrom("workspaces")
        .selectAll()
