...
export function propertyToDisplayName(propertyUrl) {
    if (propertyUrl.startsWith("sc-domain:"))
...
export async function importSitesFromGsc(userId, workspace) {
    ...
    await logEvent(`Zaimportowano strony z Search Console: ${result.created} nowych, ${result.updated} zaktualizowanych.`, { workspaceId: workspace.id, category: "sites", details: result });
    return result;
}
export async function createSite(workspaceId, propertyUrl) {
...
export async function verifySiteAccess(userId, site) {
    const token = await getAccessToken(userId);
...
export function listSites(workspaceId) {
    return db
        .selectFrom("sites")
        .selectAll()
        .where("workspace_id", "=", workspaceId)
...
export function getSite(siteId, workspaceId) {
    return db
        .selectFrom("sites")
        .selectAll()
        .where("id", "=", siteId)
...
export async function deleteSite(siteId, workspaceId) {
