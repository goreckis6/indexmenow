...
export async function handleGateLogin(req, res) {
    ...
    if (!got) {
        console.warn(`Gate login: puste haslo w body (keys=${body ? Object.keys(body).join(",") : "no-body"} content-type=${req.headers["content-type"] || "-"})`);
        renderGate(res, "Nieprawidlowe haslo.");
        return;
    }
    if (!passwordMatches(got, expected)) {
        console.warn(`Gate login fail: got_len=${got.length} exp_len=${expected.length} ` +
    ...
    const nextUrl = typeof body?.next === "string" && body.next.startsWith("/") && !body.next.startsWith("//")
        ? body.next
        : "/";
    applyNoStore(res);
...
export function siteGateHealth() {
