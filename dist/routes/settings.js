...
settingsRouter.get("/", asyncHandler(async (req, res) => {
    ...
    const siteCount = await db
        ...
        .where("workspace_id", "=", workspace.id)
        .executeTakeFirst();
    res.render("settings.html", baseContext(req, {
        user,
        workspace,
...
settingsRouter.post("/workspace/create", asyncHandler(async (req, res) => {
...
settingsRouter.post("/service-account", asyncHandler(async (req, res) => {
    const workspace = req.workspace;
    const file = req.file;
    if (!file?.buffer) {
...
settingsRouter.post("/service-account/:accountId/toggle", asyncHandler(async (req, res) => {
    const account = await db
        ...
        .executeTakeFirst();
    if (!account)
        flash(req, "Nie znaleziono konta serwisowego.", "error");
    else {
        await db
...
settingsRouter.post("/service-account/:accountId/delete", asyncHandler(async (req, res) => {
...
settingsRouter.post("/test-connection", asyncHandler(async (req, res) => {
    try {
        ...
        flash(req, `Polaczenie dziala. Konto ${req.user.email} ma dostep do ${sites.length} wlasciwosci w Search Console.`, "success");
    }
    catch (error) {
        const message = error instanceof GoogleApiError ? error.toString() : String(error);
