...
import { JobStatus } from "../db/types.js";
...
sitesRouter.get("/", asyncHandler(async (req, res) => {
    ...
    res.render("sites.html", baseContext(req, { user: req.user, workspace, rows, active_page: "sites" }));
}));
sitesRouter.post("/import", asyncHandler(async (req, res) => {
    try {
        const result = await siteService.importSitesFromGsc(req.user.id, req.workspace);
        flash(req, `Import zakonczony: ${result.created} nowych, ${result.updated} zaktualizowanych` +
...
sitesRouter.post("/add", asyncHandler(async (req, res) => {
...
sitesRouter.post("/run-all", asyncHandler(async (req, res) => {
    const workspace = req.workspace;
    const started = runInBackground(`workspace:${workspace.id}`, () => tasks.taskRunAllSites(workspace.id));
    flash(req, started ? "Uruchomiono indeksowanie wszystkich stron." : "Indeksowanie juz trwa.", started ? "success" : "warning");
    res.redirect(303, "/sites");
}));
sitesRouter.get("/:siteId", asyncHandler(async (req, res) => {
    ...
    const jobs = await db
        ...
        .limit(30)
        .execute();
    res.render("site_detail.html", baseContext(req, {
        user: req.user,
...
sitesRouter.post("/:siteId/settings", asyncHandler(async (req, res) => {
...
sitesRouter.post("/:siteId/regenerate-key", asyncHandler(async (req, res) => {
    const site = await requireSite(req, Number(req.params.siteId));
    await db
        .updateTable("sites")
...
sitesRouter.get("/:siteId/indexnow-key", asyncHandler(async (req, res) => {
    const site = await requireSite(req, Number(req.params.siteId));
    const key = site.indexnow_key || "";
    res.setHeader("Content-Disposition", `attachment; filename="${key}.txt"`);
    res.type("text/plain").send(key);
}));
sitesRouter.post("/:siteId/scan", asyncHandler(async (req, res) => {
    const site = await requireSite(req, Number(req.params.siteId));
    const started = runInBackground(`site:${site.id}`, () => tasks.taskScanSitemaps(site.id));
    flash(req, started ? "Skanowanie sitemap uruchomione w tle." : "Zadanie dla tej strony juz trwa.", started ? "success" : "warning");
    res.redirect(303, back(site.id, "sitemaps"));
}));
sitesRouter.post("/:siteId/inspect", asyncHandler(async (req, res) => {
    const site = await requireSite(req, Number(req.params.siteId));
...
sitesRouter.post("/:siteId/run", asyncHandler(async (req, res) => {
    const site = await requireSite(req, Number(req.params.siteId));
...
sitesRouter.post("/:siteId/sitemaps/add", asyncHandler(async (req, res) => {
    const site = await requireSite(req, Number(req.params.siteId));
    ...
    if (formBool(req.body.submit_to_google)) {
        ...
        flash(req, job.message || "Zgloszono sitemape.", job.status === JobStatus.SUCCESS ? "success" : "error");
    }
    res.redirect(303, back(site.id, "sitemaps"));
}));
sitesRouter.post("/:siteId/sitemaps/sync", asyncHandler(async (req, res) => {
    const site = await requireSite(req, Number(req.params.siteId));
    try {
        const result = await sitemapService.syncFromGsc(req.user.id, site);
        flash(req, `Zsynchronizowano sitemapy z GSC: ${result.created} nowych, ${result.updated} zaktualizowanych.`, "success");
    }
    catch (error) {
        const message = error instanceof GoogleApiError ? error.toString() : String(error);
...
sitesRouter.post("/:siteId/sitemaps/discover", asyncHandler(async (req, res) => {
    const site = await requireSite(req, Number(req.params.siteId));
    const found = await sitemapService.discoverSitemaps(site);
    flash(req, found.length ? `Znaleziono ${found.length} sitemap.` : "Nie znaleziono zadnej sitemapy.", found.length ? "success" : "warning");
    res.redirect(303, back(site.id, "sitemaps"));
}));
sitesRouter.post("/:siteId/sitemaps/:sitemapId/submit", asyncHandler(async (req, res) => {
    const site = await requireSite(req, Number(req.params.siteId));
...
sitesRouter.post("/:siteId/sitemaps/:sitemapId/scan", asyncHandler(async (req, res) => {
    const site = await requireSite(req, Number(req.params.siteId));
    const sitemap = await sitemapService.getSitemap(Number(req.params.sitemapId), site.id);
    if (!sitemap) {
        flash(req, "Nie znaleziono sitemapy.", "error");
    ...
    if (result.error)
        flash(req, `Blad skanowania: ${result.error}`, "error");
    else {
        flash(req, `Zaimportowano ${result.added} nowych URL-i (${result.duplicates} juz istnialo).`, "success");
    }
    res.redirect(303, back(site.id, "sitemaps"));
}));
sitesRouter.post("/:siteId/sitemaps/:sitemapId/delete", asyncHandler(async (req, res) => {
    const site = await requireSite(req, Number(req.params.siteId));
    const sitemap = await sitemapService.getSitemap(Number(req.params.sitemapId), site.id);
    if (!sitemap) {
        flash(req, "Nie znaleziono sitemapy.", "error");
    ...
    if (formBool(req.body.from_google)) {
        ...
        if (job.status !== JobStatus.SUCCESS)
            flash(req, `Google: ${job.message}`, "warning");
    }
