...
import { asyncHandler, loadUser, requireApiAuth, requireSite, resolveWorkspace, } from "../middleware/auth.js";
...
apiRouter.get("/overview", asyncHandler(async (req, res) => {
...
apiRouter.get("/tasks", asyncHandler(async (req, res) => {
...
apiRouter.get("/sites/:siteId/summary", asyncHandler(async (req, res) => {
...
apiRouter.get("/urls/:urlId", asyncHandler(async (req, res) => {
    const page = await db
        .selectFrom("urls")
...
apiRouter.get("/status-breakdown", asyncHandler(async (req, res) => {
    const rows = await db
        .selectFrom("urls")
        .innerJoin("sites", "sites.id", "urls.site_id")
        .select(["urls.index_status as status", sql `COUNT(urls.id)`.as("total")])
        .where("sites.workspace_id", "=", req.workspace.id)
        .groupBy("urls.index_status")
        .execute();
    const counts = Object.fromEntries(Object.values(IndexStatus).map((s) => [s, 0]));
