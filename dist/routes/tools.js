...
toolsRouter.get("/", asyncHandler(async (req, res) => {
    res.render("tools.html", baseContext(req, {
        user: req.user,
...
toolsRouter.post("/instant", asyncHandler(async (req, res) => {
    ...
    const jobType = req.body.notification_type === "URL_DELETED" ? JobType.URL_DELETED : JobType.URL_UPDATED;
...
toolsRouter.post("/indexnow", asyncHandler(async (req, res) => {
    const site = await requireSite(req, Number(req.body.site_id));
    const candidates = parseUrlBlob(String(req.body.urls_blob ?? ""));
    if (candidates.length === 0) {
        flash(req, "Nie podano poprawnych adresow URL.", "error");
...
toolsRouter.get("/preview", asyncHandler(async (req, res) => {
...
toolsRouter.get("/sitemap", asyncHandler(async (req, res) => {
    const url = typeof req.query.url === "string" ? req.query.url : "";
    let result = null;
    if (url) {
        const normalized = normalizeUrl(url);
