...
async function postForm(endpoint: string, form: Record<string, string>): Promise<TokenResponse> {
...
export function getCredential(userId: number): Promise<GoogleCredential | undefined> {
  return db
...
export async function storeCredentials(
  ...
  const values = {
    access_token_enc: encrypt(tokenData.access_token) as string,
  ...
  if (!credential) throw new GoogleApiError("Nie udalo sie zapisac tokenow Google.");
  return credential;
}
...
export async function getAccessToken(userId: number): Promise<string> {
  ...
  const freshEnough =
    credential.expires_at !== null && credential.expires_at.getTime() - 90_000 > Date.now();

  if (token && freshEnough) return token;
...
export function scopeList(credential: GoogleCredential): string[] {
  return credential.scopes.split(" ").filter(Boolean);
}

export async function hasScope(userId: number, scope: string): Promise<boolean> {
  const credential = await getCredential(userId);
