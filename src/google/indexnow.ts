...
export async function submitUrls(
  ...
  if (!host) throw new GoogleApiError(`Nieprawidlowy URL: ${first}`);
  ...
  const endpoint = ENDPOINTS[engine] ?? (ENDPOINTS["generic"] as string);
...
export function keyFileUrl(siteHomeUrl: string, key: string): string {
  try {
