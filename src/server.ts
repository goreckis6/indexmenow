...
async function buildFullApp(): Promise<express.Express> {
  ...
  app.use(
    "/static",
    express.static(publicDir, { maxAge: "7d", fallthrough: true, index: false }),
  );
  if (publicDirCwd !== publicDir) {
    app.use(
      "/static",
      express.static(publicDirCwd, { maxAge: "7d", fallthrough: false, index: false }),
    );
  }
  ...
  const { siteGate, handleGateLogin, handleGateLogout, hasGateAccess, siteGateHealth } =
    await import("./middleware/siteGate.js");
