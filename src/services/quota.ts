...
export async function serviceAccountCapacity(workspaceId: number): Promise<number> {
  const row = await db
    .selectFrom("service_accounts")
    .select((eb) => eb.fn.sum<number>("daily_quota").as("total"))
    .where("workspace_id", "=", workspaceId)
 ...
 */
export async function syncWorkspaceQuota(workspaceId: number): Promise<{
  dailyQuota: number;
  serviceAccounts: number;
  oauthBaseline: number;
}> {
...
export function defaultSiteDailyLimit(workspaceQuota = config.defaultDailyQuota): number {
