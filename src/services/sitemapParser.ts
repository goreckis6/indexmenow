...
function emptyResult(source: string): SitemapResult {
...
export function parseSitemapXml(content: Buffer, source: string): SitemapResult {
  ...
  if (index && typeof index === "object") {
    result.isIndex = true;
    for (const item of asArray((index as Record<string, unknown>)["sitemap"])) {
      const loc = textOf((item as Record<string, unknown>)?.["loc"]);
      if (loc) result.childSitemaps.push(loc);
    }
    return result;
  }
  ...
  if (urlset && typeof urlset === "object") {
    for (const item of asArray((urlset as Record<string, unknown>)["url"])) {
...
export async function crawlSitemap(
  ...
  if (seen.has(url) || depth > MAX_DEPTH) return aggregate;
  seen.add(url);
  ...
  for (const childUrl of parsed.childSitemaps) {
    if (aggregate.entries.length >= MAX_URLS) break;
...
export async function readRobotsSitemaps(homeUrl: string): Promise<string[]> {
...
export async function guessSitemapUrls(homeUrl: string): Promise<string[]> {
...
export function urlBelongsToSite(
  url: string,
  homeUrl: string,
